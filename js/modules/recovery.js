    ...
    async function renderTable(cases) {
        ...
        if (filtered.length === 0) {
            ...
            filtered.forEach((c, index) => {
                ...
                row.innerHTML = `
                    <td class="px-6 py-4 text-slate-400 font-normal">${index + 1}</td>
                    <td class="px-6 py-4">
                        <div class="font-normal text-slate-900">${c.accountNo}</div>
                    </td>
                    <td class="px-6 py-4">
                        <div class="text-slate-500 font-normal text-xs">L..${c.name}</div>
                        <div class="text-[10px] text-slate-400">${c.loanType} • ${c.interestRate}</div>
                    </td>
                    <td class="px-6 py-4 text-center">
                        <span class="px-4 py-1 rounded-full text-[9px] font-normal uppercase tracking-wider ${getStatusClass(c.status)}">
                            ${c.status}
                        </span>
                    </td>
    ...
    window.deleteRecoveryCase = async (id) => {
        if (await window.showConfirm('Delete Case?', 'Are you sure you want to delete this recovery case?', 'Yes, delete')) {
            try {
                ...
                window.showToast('Recovery case deleted.', 'warning');
                loadRecoveryData();
