    ...
    async function loadStudents() {
        ...
        try {
            ...
            filtered.forEach(student => {
                ...
                tr.innerHTML = `
                    <td class="px-5 py-3 border-b border-slate-100 font-normal text-slate-800">${student.studentName}</td>
                    <td class="px-5 py-3 border-b border-slate-100 text-slate-600">${student.schoolName}</td>
                    <td class="px-5 py-3 border-b border-slate-100 text-center"><span class="bg-indigo-50 text-indigo-700 font-normal px-2 rounded">Gr ${student.grade}</span></td>
            ...
            let html = `
                <div class="mt-4 border-t border-slate-200 pt-4 animate__animated animate__fadeIn">
                    <div class="flex justify-between items-center mb-3">
                        <h5 class="text-sm font-normal text-slate-800">Recipients Ready (${targets.length})</h5>
                        <button onclick="window.copyAllNumbers('${allNumbers}')" class="text-[10px] bg-slate-100 hover:bg-slate-200 text-slate-600 px-2 py-1 rounded font-normal transition-colors">
                ...
                html += `
                    <div class="flex justify-between items-center bg-slate-50 p-2 rounded-lg border border-slate-100 hover:border-slate-200 transition-colors">
                        <div class="flex flex-col">
                            <span class="text-xs font-normal text-slate-700 leading-tight">${t.studentName}</span>
                            <span class="text-[10px] text-slate-500">Gr ${t.grade} • +${t.contactNumber}</span>
                        </div>
                        <a href="${link}" target="_blank" class="${btnClass} text-white px-3 py-1.5 rounded-lg text-xs font-normal transition-all shadow-md flex items-center">
