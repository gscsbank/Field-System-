        ...
        try {
            ...
            customers.forEach(cust => {
                ...
                const markerIcon = L.divIcon({
                    className: 'custom-div-icon',
                    html: `<div class='text-white rounded-full w-7 h-7 flex items-center justify-center border-2 border-white shadow shadow-blue-900/50 text-xs font-normal relative group' style='background-color: ${markerColor}'>
                            <i class="ph-fill ph-storefront"></i>
                            <div class="print-label hidden absolute top-full mt-1 px-1 bg-white text-black text-[10px] font-mono border border-slate-300 font-normal whitespace-nowrap z-50">${cust.accountNo}</div>
                           </div>`,
                    iconSize: [28, 28],
                    iconAnchor: [14, 14],
                    popupAnchor: [0, -14]
                ...
                const popupContent = `
                    <div class="p-1 min-w-[200px]">
                        <h4 class="font-normal text-slate-800 text-sm mb-1">${cust.name}</h4>
                        ...
                        <div class="flex justify-between items-center text-xs">
                            ...
                            <span class="font-normal text-slate-800">${cust.officerName}</span>
                        </div>
                        <div class="flex justify-between items-center text-xs mt-1 mb-3">
                            <span class="text-slate-500">Frequency:</span>
                            <span class="bg-blue-50 text-blue-700 px-2 py-0.5 rounded font-normal">${cust.visitFrequency}</span>
                        </div>
                        <div class="flex justify-end gap-2 border-t border-slate-100 pt-2 pb-1 print-hide">
                            <button onclick="window.editMapLocation(${cust.id})" class="text-xs text-blue-600 hover:text-blue-800 font-normal px-2 py-1 rounded bg-blue-50 hover:bg-blue-100 transition-colors">Edit</button>
                            <button onclick="window.deleteMapLocation(${cust.id})" class="text-xs text-red-600 hover:text-red-800 font-normal px-2 py-1 rounded bg-red-50 hover:bg-red-100 transition-colors">Delete</button>
                        </div>
                    </div>
                `;
        ...
        try {
            ...
            if (confirmed) {
                ...
                if (window.SyncManager && window.SyncManager.deleteFromCloud) {
                    await window.SyncManager.deleteFromCloud('customers', id);
                }
            }
        } catch (e) { console.error(e); }
    };
