...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function renderPivotTable(logs, year) {
        ...
        pivotHead.innerHTML = `
            <tr>
                <th class="px-6 py-4 text-left text-slate-600 font-normal border-b border-slate-200 whitespace-nowrap w-[25%]">Month</th>
                <th class="px-6 py-4 text-center text-primary font-normal border-b border-slate-200 bg-blue-50" colspan="${activeOfficers.length}">Field Officer</th>
                <th class="px-6 py-4 text-right text-slate-700 font-normal border-b border-slate-200 whitespace-nowrap w-[25%]">Field Saving Total</th>
                <th class="px-6 py-4 text-center border-b border-slate-200 print-hide w-12"></th>
            </tr>
            <tr>
                <th class="px-6 py-2 text-left text-slate-500 text-xs font-normal border-b border-slate-100"></th>
                ${activeOfficers.map(o => `<th class="px-6 py-2 text-center text-slate-600 text-xs font-normal border-b border-slate-100 bg-blue-50/50">${o}</th>`).join('')}
        ...
        pivotBody.innerHTML = MONTH_KEYS.map((mk, idx) => {
            ...
            const editBtns = activeOfficers.map(o => {
                ...
                if (logEntry) {
                    return `
                        <button onclick="window.editSavingsLog(${logEntry.id})" class="text-slate-400 hover:text-primary transition-colors" title="Edit ${o}"><i class="ph ph-pencil-simple text-sm"></i></button>
                        <button onclick="window.deleteSaving(${logEntry.id})" class="text-slate-400 hover:text-red-500 transition-colors" title="Delete ${o}"><i class="ph ph-trash text-sm"></i></button>
                    `;
                }
                return '';
            }).join('');
            ...
            return `
                <tr class="${bg} hover:bg-blue-50/30 transition-colors">
                    <td class="px-6 py-3 font-normal text-slate-700 whitespace-nowrap">${monthName}</td>
                    ${cells}
                    <td class="px-6 py-3 text-right font-normal text-blue-700 font-mono text-sm whitespace-nowrap">${totalDisplay}</td>
        ...
        const officerTotalCells = activeOfficers.map(o => {
            ...
            return `<td class="px-6 py-4 text-right text-slate-800 font-normal font-mono text-sm border-t-2 border-slate-300">${t > 0 ? t.toLocaleString('en-US', { minimumFractionDigits: 2 }) : ''}</td>`;
        }).join('');

        pivotFoot.innerHTML = `
            <tr class="bg-slate-100">
                <td class="px-6 py-4 font-normal text-slate-800 text-sm border-t-2 border-slate-300">Annual Field Saving Collection Total</td>
                ${officerTotalCells}
                <td class="px-6 py-4 text-right font-normal text-blue-800 font-mono border-t-2 border-slate-300 text-sm">${grandTotal.toLocaleString('en-US', { minimumFractionDigits: 2 })}</td>
    ...
    async function handleSavingsPrint() {
        ...
        let reportHTML = `
            <div style="width: 210mm; min-height: 297mm; margin: auto; padding: 40px 50px; font-family: 'Arial', 'Helvetica', sans-serif; color: #000; background: #fff; box-sizing: border-box;">
                <!-- Official Header -->
                <div style="display:flex; align-items:flex-end; justify-content:space-between; border-bottom:1px solid #000; padding-bottom:12px; margin-bottom:24px;">
                    <div style="display:flex; align-items:center; gap:16px;">
                        <img src="img/logo.png" alt="GSCS" style="height:48px; width:auto; filter:grayscale(100%);" onerror="this.style.display='none'">
                        <div>
                            <div style="font-size:17px; font-weight:600; letter-spacing:0.5px; color:#000; text-transform:uppercase; line-height:1.2;">GALAPITIYAGAMA SANASA BANK</div>
                            <div style="font-size:9px; color:#555; text-transform:uppercase; margin-top:2px; letter-spacing:1.5px; font-weight:400;">Loan Recovery Management System</div>
                        </div>
                    </div>
                    <div style="text-align:right;">
                        <div style="font-size:11px; font-weight:600; letter-spacing:1px; text-transform:uppercase; color:#000;">FIELD SAVINGS PROGRESS — ${selectedYear}</div>
                        <div style="font-size:9px; color:#666; margin-top:4px; font-weight:400;">Generated: ${new Date().toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}</div>
                    </div>
                </div>
