            ...
            allEvents.forEach(evt => {
                ...
                tr.innerHTML = `
                    <td class="px-5 py-4 border-b border-slate-100 border-r border-slate-50 relative">
                        ...
                        <h5 class="font-normal text-slate-800 mb-1 ml-2">${evt.title}</h5>
                    ...
                    <td class="px-5 py-4 border-b border-slate-100 text-center align-middle border-r border-slate-50">
                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-normal ${statusTheme}">
                            ${evt.status}
                        </span>
                    </td>
                    <td class="px-5 py-4 border-b border-slate-100 align-middle">
                        <div class="flex flex-col space-y-2 lg:flex-row lg:space-y-0 lg:space-x-2 justify-center print-hide mb-2">
                            ${evt.status === 'Pending' ? `<button onclick="window.updateEventStatus(${evt.id}, 'Manager Approved')" class="px-3 py-1 bg-blue-50 text-blue-700 hover:bg-blue-100 rounded text-xs font-normal transition-colors w-full lg:w-auto">Approve (Mgr)</button>` : ''}
                            ${evt.status === 'Manager Approved' ? `<button onclick="window.updateEventStatus(${evt.id}, 'Board Approved')" class="px-3 py-1 bg-emerald-50 text-emerald-700 hover:bg-emerald-100 rounded text-xs font-normal transition-colors w-full lg:w-auto">Approve (Board)</button>` : ''}
                            ${(evt.status !== 'Rejected' && evt.status !== 'Board Approved') ? `<button onclick="window.updateEventStatus(${evt.id}, 'Rejected')" class="px-3 py-1 bg-red-50 text-red-700 hover:bg-red-100 rounded text-xs font-normal transition-colors w-full lg:w-auto">Reject</button>` : ''}
                            <button onclick="window.generateEventPDF(${evt.id})" class="px-3 py-1 bg-slate-100 text-slate-700 hover:bg-slate-200 rounded text-xs font-normal transition-colors w-full lg:w-auto flex items-center justify-center"><i class="ph ph-printer mr-1"></i> Print</button>
                        </div>
                ...
                <div class="pdf-container" style="width: 210mm; min-height: 297mm; margin: auto; padding: 30mm 20mm; font-family: 'Arial', sans-serif; background: white; box-sizing: border-box;">
                    ...
                    <div style="display:flex; align-items:flex-end; justify-content:space-between; border-bottom:1px solid #000; padding-bottom:12px; margin-bottom:24px;">
                        <div style="display:flex; align-items:center; gap:16px;">
                            ...
                            <div>
                                <div style="font-size:18px; font-weight:600; letter-spacing:0.5px; color:#000; text-transform:uppercase; line-height:1.2;">GALAPITIYAGAMA SANASA BANK</div>
                        ...
                        <div style="text-align:right;">
                            <div style="font-size:11px; font-weight:600; letter-spacing:1px; text-transform:uppercase; color:#000;">EVENT &amp; PROJECT PROPOSAL</div>
